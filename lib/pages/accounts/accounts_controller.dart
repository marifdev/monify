...
class AccountsController {
  ...
  //   getAccounts(_model.userId).then((value) {
  //     _model.loadAccounts(value);
  //     // _model.sortAccountsByType();
  //   });
  ...
  void addAccount(Account account) async {
    ...
    await FirestoreMethods().addAccount(account: account, uid: _model.user!.uid);
    _model.setLoading(false);
  }

  void deleteAccount(String accountId) async {
    _model.setLoading(true);
    await FirestoreMethods().deleteAccount(accountId, _model.user!.uid).then((value) => refreshAccounts());
    _model.setLoading(false);
  }

  void refreshAccounts() async {
    _model.setLoading(true);
    await getAccounts(_model.user!.uid).then((value) => _model.loadAccounts(value));
    _model.setLoading(false);
  }
}
